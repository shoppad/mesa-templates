...
module.exports = new class {
  ...
  script = (payload, context) => {
    ...
    if (context.task.source === 'ignore') {
    ...
    else if (Filter.process(a, b, comparison, additional, {setTaskStatus: false})) {
      ...
      Mesa.trigger.setTaskExternalData({label: 'Existing product is empty, creating new'});
      Mesa.output.next(payload);
    }
    else {
      ...
      Mesa.trigger.setTaskExternalData({label: 'Existing product is not empty, skipping create'});
      Mesa.output.send('transform', payload);
    }
  }
}